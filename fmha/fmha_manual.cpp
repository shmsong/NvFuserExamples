...
class FMHATrainKernel : public NVFuserOpBase<FMHATrainKernel> {
public:
  c10::optional<LaunchParams> fusedKernel(Fusion &fusion,
                                          std::vector<c10::IValue> inputs) {
    const int seql_q = 128;
    ...
    fusion.addInput(qk);
    fusion.addInput(v);
    fusion.addInput(pad_mask);
    fusion.addInput(dropout_mask);
    ...
    // Approximated masking from contrib reference.
    auto tvp_masked = add(tvp_scaled, pad_mask);
    ...
    auto recip = reciprocal(bcast_sum);
    auto tvpsfm = mul(exp_val, recip);
    auto tv_dropout_mask =
        div(dropout_mask, IrBuilder::create<Double>(dropout_prob));
    auto tv_dropout = mul(tvpsfm, tv_dropout_mask);

    auto tvph = castOp(DataType::Half, tv_dropout);
    ...
    schedule_epilog_tv(recip);
    schedule_epilog_tv(tvpsfm);
    schedule_epilog_tv(tv_dropout_mask);
    schedule_epilog_tv(tv_dropout);
    schedule_epilog_tv(tvph);

    // Put tvph result into smem:
    schedule_epilog_tv(tvphcw);

    pad_mask->computeAt(tvp_masked, -1);
    dropout_mask->computeAt(tv_dropout_mask, -1);
...
class FMHATrainKernelSaveIntermediate
    : public NVFuserOpBase<FMHATrainKernelSaveIntermediate> {
public:
  c10::optional<LaunchParams> fusedKernel(Fusion &fusion,
                                          std::vector<c10::IValue> inputs) {
    const int seql_q = 128;
    ...
    auto saved_softmax = castOp(DataType::Half, tvpsfm);
    auto saved_dropout = castOp(DataType::Half, tv_dropout);

    fusion.addOutput(tvouth);

    fusion.addOutput(saved_softmax);
    fusion.addOutput(saved_dropout);
    ...
    schedule_epilog_tv(tv_dropout);
    schedule_epilog_tv(tvph);
    schedule_epilog_tv(saved_softmax);
    schedule_epilog_tv(saved_dropout);
...
at::Tensor fmha_train_nvfuser(const at::Tensor &input, const at::Tensor &qk,
                              const at::Tensor &v, const at::Tensor &pad_mask,
                              const at::Tensor &dropout_mask) {
  auto outputs = _fmha_train_kernel.run({input, qk, v, pad_mask, dropout_mask});
  return outputs[0];
}

std::vector<at::Tensor>
fmha_train_saveim_nvfuser(const at::Tensor &input, const at::Tensor &qk,
                          const at::Tensor &v, const at::Tensor &pad_mask,
                          const at::Tensor &dropout_mask) {
  auto outputs =
      _fmha_train_kernel_save_im.run({input, qk, v, pad_mask, dropout_mask});
  return outputs;
}

TORCH_LIBRARY(mha_manual, m) {
  m.def("fmha_nvfuser", fmha_nvfuser);
  m.def("fmha_train_nvfuser", fmha_train_nvfuser);
  m.def("fmha_train_saveim_nvfuser", fmha_train_saveim_nvfuser);
}

TORCH_LIBRARY_IMPL(mha_manual, CUDA, m) {
  m.impl("fmha_nvfuser", fmha_nvfuser);
  m.impl("fmha_train_nvfuser", fmha_train_nvfuser);
  m.impl("fmha_train_saveim_nvfuser", fmha_train_saveim_nvfuser);
}
